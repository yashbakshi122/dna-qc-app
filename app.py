...
def gc_percent(seq: str) -> float:
    # ignore N in denominator (common practice)
...
def shannon_entropy(seq: str) -> float:
    # simple complexity check; low entropy => repetitive
...
def longest_homopolymer(seq: str) -> int:
    # max run of same base, e.g., AAAAAA
    best = 0
    cur = 0
    prev = ""
    for ch in seq:
...
uploaded = st.file_uploader("Upload FASTA (.fasta / .fa / .txt)", type=["fasta", "fa", "txt"])

if not uploaded:
    st.info("Upload a FASTA file to generate a QC report.")
    st.stop()
